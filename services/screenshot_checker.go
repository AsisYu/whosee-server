...
func (sc *ScreenshotChecker) TestScreenshotHealth(domain ...string) map[string]interface{} {
	...
	log.Println("[Screenshot:详细-服务] 初始化截图服务测试")
	...
	if len(domain) > 0 && domain[0] != "" {
		...
		log.Printf("[Screenshot:详细-服务] 使用用户指定的域名: %s", targetDomain)
		// 确保域名格式正确（添加协议前缀）
		...
		log.Printf("[Screenshot:详细-服务] 未提供特定域名，将测试 %d 个预设URL", len(sc.testURLs))
	}
	...
	log.Printf("[Screenshot:详细-服务] 将测试 %d 个目标URL", totalTests)

	// 测试每个URL
	for i, url := range testURLs {
		log.Printf("[Screenshot:详细-服务] 开始测试第 %d 个URL: %s", i+1, url)
		...
		startTime := time.Now()
		log.Printf("[Screenshot:详细-服务] 创建HTTP客户端，超时设置: 15秒")
		...
		if err != nil {
			log.Printf("[Screenshot:详细-服务] 创建请求失败: %v", err)
			testResult["message"] = "创建请求失败: " + err.Error()
		} else {
			req.Header.Set("User-Agent", "WhoseeMeHealthCheck/1.0")
			log.Printf("[Screenshot:详细-服务] 发送HTTP请求到 %s", url)

			resp, err := client.Do(req)
			responseTime := time.Since(startTime)
			testResult["responseTime"] = responseTime.Milliseconds()
			log.Printf("[Screenshot:详细-服务] 收到响应，耗时: %v", responseTime)

			if err != nil {
				log.Printf("[Screenshot:详细-服务] 请求失败: %v", err)
				testResult["message"] = "请求失败: " + err.Error()
			} else {
				// 2xx或3xx状态码都认为是成功
				if resp.StatusCode >= 200 && resp.StatusCode < 400 {
					log.Printf("[Screenshot:详细-服务] 请求成功，状态码: %d", resp.StatusCode)
					testResult["success"] = true
					testResult["message"] = "网站可访问"
					testResult["statusCode"] = resp.StatusCode
					successfulTests++
				} else {
					log.Printf("[Screenshot:详细-服务] 网站返回错误状态码: %s", resp.Status)
		...
		// 添加测试结果
		log.Printf("[Screenshot:详细-服务] 保存第 %d 个URL的测试结果", i+1)
	...
	log.Printf("[Screenshot:详细-服务] 测试结束，成功率: %d/%d", successfulTests, totalTests)
	...
	log.Printf("[Screenshot:详细-服务] 最终状态: %s", map[bool]string{true: "可用", false: "不可用"}[serviceAvailable])
